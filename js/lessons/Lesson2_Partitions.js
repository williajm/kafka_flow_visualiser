/**
 * Lesson 2: Partitions
 * Demonstrates how messages distribute across multiple topic partitions
 * Shows ordering guarantees within partitions
 */
...
export class Lesson2_Partitions extends Scene {
    constructor(canvas) {
        super(canvas, {
            title: 'Partitions',
            description: 'Topics are divided into partitions for parallel processing. Messages are distributed across partitions, with ordering guaranteed within each partition.'
        });
     ...
     */
    async setup() {
        // Create entities - topic now has 3 partitions
        this.producer = new Producer('producer-1', 100, 260);
        this.topic = new Topic('topic-1', 500, 200, this.partitionCount);
    ...
    setupClickHandlers() {
        ...
        if (topicEl) {
            topicEl.addEventListener('click', () => {
                ...
                info.description += ' In this lesson, messages are distributed round-robin across partitions.';
        ...
        if (consumerEl) {
            consumerEl.addEventListener('click', () => {
                const info = this.consumer.getInfo();
                info.description += ' This consumer reads from all partitions.';
                eventBus.emit('entity:click', info);
            });
        }
    }
    ...
    createAnimationTimeline() {
        ...
        // Create 9 messages (3 per partition) that flow through the system
        for (let i = 0; i < 9; i++) {
            const delay = i * 0.8; // Stagger messages
            const partitionIndex = i % this.partitionCount; // Round-robin distribution

            timeline.add(() => {
                this.createAndAnimateMessage(partitionIndex);
            }, delay);
        }
    ...
    /**
     * Create and animate a single message through a specific partition
     * @param {number} partitionIndex
     */
    createAndAnimateMessage(partitionIndex) {
